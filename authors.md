# Authors and Citation

## Authors

- **Alex M Chubaty**. Author, maintainer.
  [](https://orcid.org/0000-0001-7146-8135)

- **Ceres Barros**. Author. [](https://orcid.org/0000-0003-4036-977X)

- **Eliot J B McIntire**. Author.
  [](https://orcid.org/0000-0002-6914-8316)

- **His Majesty the King in Right of Canada, as represented by the
  Minister of Natural Resources Canada**. Copyright holder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/PredictiveEcology/SpaDES.docs/blob/v0.5.0/DESCRIPTION)

Chubaty A, Barros C, McIntire E (2026). *SpaDES.docs: Helper Utilities
for Creating SpaDES Module Documentation and Manuals*. R package version
0.5.0, <https://predictiveecology.github.io/SpaDES.docs/>.

    @Manual{,
      title = {SpaDES.docs: Helper Utilities for Creating SpaDES Module Documentation and Manuals},
      author = {Alex M Chubaty and Ceres Barros and Eliot J B McIntire},
      year = {2026},
      note = {R package version 0.5.0},
      url = {https://predictiveecology.github.io/SpaDES.docs/},
    }
